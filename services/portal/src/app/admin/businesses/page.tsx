    ...
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className={`flex flex-col border-r bg-white transition-all duration-200 ${
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center text-gray-400 py-16 text-sm">{t('businesses', 'noBusinesses')}</div>
          ) : (
            <div className="divide-y">
              {filtered.map(biz => (
                <button
                  ...
                  className={`w-full text-left px-4 sm:px-6 py-4 hover:bg-gray-50 transition ${
                    selectedId === biz.id ? 'bg-brand-50 border-l-4 border-brand-600' : ''
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                        ...
                        {biz.email && (
                          <span className="text-xs text-gray-500 truncate">{biz.email}</span>
                        )}
                      </div>
                    </div>
      ...
      {selectedBusiness ? (
        <div className={`flex-1 overflow-y-auto bg-gray-50 ${
          ...
          <div className="p-4 sm:p-6 space-y-6">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-gray-500 flex-wrap">
                    {selectedBusiness.email && (
                      <a href={`mailto:${selectedBusiness.email}`} className="hover:text-brand-600 transition truncate">{selectedBusiness.email}</a>
                    )}
                    {selectedBusiness.phone && (
                      <a href={`tel:${selectedBusiness.phone}`} className="hover:text-brand-600 transition">{selectedBusiness.phone}</a>
                    )}
                    {selectedBusiness.website && (
                      <a href={selectedBusiness.website} target="_blank" rel="noopener noreferrer" className="hover:text-brand-600 transition truncate">{selectedBusiness.website}</a>
                    )}
                  </div>
                </div>
                <button
            ...
            <div className="bg-white rounded-xl shadow-sm border p-4 sm:p-6 space-y-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('businesses', 'editDetails')}</h3>

              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'name')}</label>
                <input
                  type="text"
                  value={detailName}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'address')}</label>
                <input
                  type="text"
                  value={detailAddress}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'city')}</label>
                  <input
                    type="text"
                    value={detailCity}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'province')}</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'postalCode')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'country')}</label>
                  <input
                    type="text"
                    value={detailCountry}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'phone')}</label>
                  <input
                    type="tel"
                    value={detailPhone}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'email')}</label>
                  <input
                    type="email"
                    value={detailEmail}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'website')}</label>
                <input
                  type="url"
                  value={detailWebsite}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('businesses', 'notes')}</label>
                <textarea
                  value={detailNotes}
            ...
            {/* Visit history */}
            <div className="bg-white rounded-xl shadow-sm border p-4 sm:p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('businesses', 'visitHistory')}</h3>
              {detailLoading ? (
                <div className="flex items-center justify-center h-20">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-brand-600" />
                </div>
              ) : visitGroups.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-6">{t('businesses', 'noVisits')}</p>
              ) : (
                <div className="space-y-3">
                  {visitGroups.map(vg => (
                    <div key={vg.id} className="bg-gray-50 rounded-lg p-3 border">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-gray-900">
                          {vg.visitors?.map(v => v.name).join(', ') || t('businesses', 'unknownVisitor')}
                        </span>
                        <span className="text-[10px] text-gray-400">{formatDate(vg.visitedAt)}</span>
                      </div>
                      {vg.notes && <p className="text-xs text-gray-600 mt-1">{vg.notes}</p>}
                    </div>
                  ))}
                </div>
              )}
            ...
            {/* Files section */}
            <div className="bg-white rounded-xl shadow-sm border p-4 sm:p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('businesses', 'files')}</h3>
              {detailLoading ? (
                <div className="flex items-center justify-center h-20">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-brand-600" />
                </div>
              ) : files.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-6">{t('businesses', 'noFiles')}</p>
              ) : (
                <div className="space-y-2">
                  {files.map(file => (
                    <a
                      ...
                      className="flex items-center gap-3 bg-gray-50 rounded-lg p-3 border hover:bg-gray-100 transition"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-gray-400">{formatDate(file.uploadedAt)}</p>
                      </div>
                    </a>
                  ))}
                </div>
        ...
        <div className="flex-1 hidden md:flex items-center justify-center bg-gray-50">
          <p className="text-gray-400 text-sm">{t('businesses', 'selectBusiness')}</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-900">{t('businesses', 'newBusiness')}</h3>
              <button onClick={() => setShowNewBusiness(false)} className="p-1 text-gray-400 hover:text-gray-600 transition">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
