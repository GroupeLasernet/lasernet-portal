...
export default function ClientFilesPage() {
  ...
  const fileIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <div className="w-10 h-10 bg-gray-50 dark:bg-gray-900 rounded-lg flex items-center justify-center text-gray-400 dark:text-gray-500 font-bold text-xs">FILE</div>;
    }
  };
    ...
    <div>
      ...
      {myFiles.length > 0 ? (
          ...
          {myFiles.map(file => (
            <div key={file.id} className="card flex items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">{file.category} • {file.size} • Uploaded {file.uploadedAt}</p>
              </div>
        ...
        <div className="card text-center py-12">
          ...
          <p className="text-gray-500 dark:text-gray-400">No files available yet.</p>
        </div>
      )}
    </div>
  );
