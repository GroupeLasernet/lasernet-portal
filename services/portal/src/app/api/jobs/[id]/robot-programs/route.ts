...
// GET /api/jobs/[id]/robot-programs — List robot programs for a job
export async function GET(request: NextRequest, { params }: { params: RouteParams }) {
  try {
    ...
    const programs = await prisma.jobRobotProgram.findMany({
      where: { jobId: id },
...
// POST /api/jobs/[id]/robot-programs — Save a robot program to a job
export async function POST(request: NextRequest, { params }: { params: RouteParams }) {
  try {
    ...
    const program = await prisma.jobRobotProgram.create({
      data: {
        jobId: id,
        name,
