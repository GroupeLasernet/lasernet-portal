...
export async function GET(req: NextRequest, { params }: { params: { path: string[] } }) {
  ...
  try {
    const res = await fetch(`${RELFAR_URL}${path}`, {
    ...
    return NextResponse.json(
      { error: 'Relfar service unreachable', details: e.message },
      { status: 503 }
    );
  }
}
...
export async function POST(req: NextRequest, { params }: { params: { path: string[] } }) {
  ...
  // For control operations (connect, disconnect, write, scan), verify admin role
  const path = '/' + params.path.join('/');
  const controlOps = ['/connect', '/disconnect', '/write', '/scan'];
