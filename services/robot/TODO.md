...
- [ ] **Speed multiplier slider** *(2026-04-14)* — UI slider added below the 0–100% Speed slider. Multiplies the 180 mm/s · 180 °/s base ceiling by the multiplier value. Hugo to decide on the multiplier's **max value** (TBD) based on safe motion limits for Elfin E03 Pro. Interim default: max=3× so we don't ship anything too fast by accident. Backend hard caps in `robot_comm.py` raised 180 → 540 provisionally to match; revise once final ceiling is agreed.
