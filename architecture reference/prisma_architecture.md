...
- `FileAsset` — `{ driveFileId, name, mimeType, sizeBytes, folderId?, scope, managedClientId?, localBusinessId? }` + legacy `category` / `subCategory` string columns still on the table for backward-read compatibility (shim during rollout; write path nulls them whenever `folderId` is set). `folderId` → `FileFolder(id)` with `ON DELETE SET NULL`. Related: `FileAssetSku[]` (see below).
- `VideoAsset` — Vimeo-linked, no storage on our side. Same folder treatment as `FileAsset`: `folderId` FK + legacy category strings. Related: `VideoAssetSku[]` (see below).
...
- `FileAssetSku` / `VideoAssetSku` — Many-to-many join between files/videos and QuickBooks inventory items. `{ fileAssetId|videoAssetId, skuId, skuName? }` with unique compound `(fileAssetId|videoAssetId, skuId)`, `ON DELETE CASCADE` on the asset side, and no FK on `skuId` (QB items aren't mirrored into Postgres — `skuId` is the QB `Item.Id` string). Added 2026-04-20 migration `20260420b_file_video_sku_links.sql`. The `EditDocumentModal` + `VideoModal` use the shared `SkuPicker` component (fuzzy autocomplete sourced from `QuickBooksContext.inventory`) to add/remove links. Downstream the `RelatedFilesChip` on each line item of `/admin/quotes` and `/admin/invoices` calls `/api/files/by-sku?skuId=…` to surface every linked file/video.

APIs: `/api/files/documents` (GET list, POST upload multipart — accepts `folderId` + `skuIds[]`), `/api/files/documents/[id]` (PATCH rename + `folderId` move + `skuIds[]` diff-replace + Drive rename, DELETE Drive+DB), `/api/files/documents/[id]/download` (GET stream from Drive), `/api/files/videos` (GET, POST with `folderId` + `skuIds[]`), `/api/files/videos/[id]` (PATCH — accepts `skuIds[]`; DELETE), `/api/files/folders` (GET list, POST create `{name, parentId?}`), `/api/files/folders/[id]` (PATCH rename + move — cycle-guarded against self/descendant parents, sibling-unique; DELETE — BFS the subtree, null `folderId` on every contained FileAsset/VideoAsset, then delete the root — CASCADE handles descendant folders), `/api/files/by-sku?skuId=…|?skuIds=a,b,c` (GET — returns every document + video linked to the given QB Item.Id; single-id returns flat `{documents, videos}`, batch returns `{[skuId]: {documents, videos}}`).
...
4. **Migrations** — run in order against that environment's Neon branch (localhost dev branch via `npx prisma db execute --file ...`, or production branch via Neon SQL editor):
   ...
   4. `20260420b_file_video_sku_links.sql` (many-to-many SKU join tables — `FileAssetSku` + `VideoAssetSku`). **Idempotent** — `CREATE TABLE IF NOT EXISTS`, safe to re-run.
...
- **Phase 4 — `translations.ts` split (commit `1532915`):** the 1128-line monolith was split into 17 per-namespace modules under `src/lib/i18n/` (`common`, `nav`, `login`, `forgot`, `dashboard`, `clients`, `stations`, `machines`, `trainingPage`, `tickets`, `files`, `settings`, `stationPcs`, `leads`, `emails`, `businesses`, `liveVisits`). `src/lib/i18n/index.ts` imports each namespace, reassembles the `translations` object, and re-exports `t()` + `Language` + `SectionKey`. `src/lib/translations.ts` is now a 16-line barrel that re-exports the same public API — all four existing consumers (`LanguageContext.tsx`, `api/training/events`, `api/invite`, `api/reset-password`) keep working with zero changes. Adding a new namespace is now: (1) create `./<name>.ts`, (2) import + add one line to `i18n/index.ts`. No schema.prisma split (Prisma's `prismaSchemaFolder` is a preview feature with risk to migrations + generator — deferred; schema stays as one 1231-line file with domain-marker comments for now).

**Files page — container kebab menus (2026-04-21, branch `feat/files-container-kebabs`, commit `865527d`):** Documents + Vidéos containers on `/admin/files` gained a right-anchored ⋮ kebab menu in the header. New reusable component `src/app/admin/files/ContainerMenu.tsx` (111 lines) with discriminated-union items (`{kind:'item', label, onClick, checked?} | {kind:'divider'}`), click-outside + Escape close, ✓ prefix for active entry, absolute positioning so layout doesn't shift. `DocumentsTable` gained local `DocSort = 'newest'|'oldest'|'name-asc'|'name-desc'|'size-desc'` state + `applyDocSort()` helper (localeCompare with `numeric: true` for names, `sizeBytes` subtract for size); menu items: Téléverser dans ce conteneur → divider → 5 sort options, default newest-first. `VideosGrid` mirrors the pattern with `VideoSort` (no `size-desc` since videos have no size) + `applyVideoSort()` using `title` instead of `name`; menu items: Ajouter une vidéo Vimeo → divider → 4 sort options. Page-level callbacks wired: `onUpload={() => fileInputRef.current?.click()}` reuses the existing hidden file picker; `onAddVideo={() => setAddingVideo(true)}` reuses the existing Vimeo drawer. 8 new i18n keys appended to `src/lib/i18n/files.ts`: `menuLabel`, `sortByNewest/Oldest/NameAsc/NameDesc/Largest`, `menuUpload`, `menuAddVideo`. Verified on preview via Chrome: both kebabs render top-right, menus show correct items + active ✓, sort selection transitions the ✓, Escape + click-outside both close, each container has independent sort state.
